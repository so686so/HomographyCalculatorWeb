document.addEventListener("DOMContentLoaded", () => {
	...
	const saveGlobalJsonBtnElement =
		document.getElementById("saveGlobalJsonBtn"); // 추가됨
	...
	function deriveAndDrawAnnotations() {
		...
		// console.log('[deriveAndDrawAnnotations] Annotations to be drawn:', JSON.parse(JSON.stringify(currentImageAnnotations)));
		redrawCrosshairs();
		updateStatsHeader();
	}
	...
	function drawCrosshair(x, y, color) {
		if (
			...
			// console.warn('[drawCrosshair] Pre-conditions not met or canvas not ready.');
			return;
		}
	...
	function redrawCrosshairs() {
		if (
			...
			// console.warn("RedrawCrosshairs: Canvas or image not ready (offsetWidth/Height is 0).");
			if (ctx)
				ctx.clearRect(
					0,
					0,
		...
		// console.log('[redrawCrosshairs] Hotspots updated:', allCrosshairHotspots.length);
	}
	...
	uploadedImageElement.addEventListener("mousemove", (event) => {
		...
		if (hoveredHotspot) {
			// console.log('Tooltip - Hovered hotspot data:', JSON.parse(JSON.stringify(hoveredHotspot)));
			...
			// console.log('Tooltip - Setting innerHTML to:', `${camText}<br>${gndText}`);
	...
	uploadedImageElement.addEventListener("click", (event) => {
		...
		if (clickedHotspot) {
			...
			// console.log('Clicked hotspot for menu:', JSON.parse(JSON.stringify(clickedHotspot)));
	...
	function showCrosshairActionMenu(hotspot, clientX, clientY) {
		...
		requestAnimationFrame(() => {
			...
			// console.log('ActionMenu displayed at:', {top, left});
		});
	}
	...
	function updateStatsHeader() {
		if (statsTotalConfirmedElement)
			statsTotalConfirmedElement.textContent = globalJsonData.data.length;
		let tempCnt = 0,
			confCnt = 0;
	...
	if (saveCalibrationResultBtnElement && calibrationOutputJsonElement) {
		// Save Calibration Result 버튼 리스너
		saveCalibrationResultBtnElement.addEventListener("click", async () => {
			...
			try {
				const calibrationData = JSON.parse(jsonOutputText);
