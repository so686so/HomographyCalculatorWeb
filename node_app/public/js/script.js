document.addEventListener("DOMContentLoaded", () => {
    ...
    // Homography 페이지 요소
    const statusGlobalAnnotationsElement = document.getElementById("statusGlobalAnnotations");
    const statusCalibrationResultElement = document.getElementById("statusCalibrationResult");
    const statusCppApiServerElement = document.getElementById("statusCppApiServer");
    const refreshHomographyStatusBtnElement = document.getElementById("refreshHomographyStatusBtn");
    ...
    async function selectImage(fileObject) {
        ...
        try {
            ...
            currentImageDefinedPoints = !response.ok
                ...
                : await response.json();
            console.log(
                `[selectImage] Fetched ${currentImageDefinedPoints
    ...
    uploadedImageElement.addEventListener("mousemove", (event) => {
        ...
        if (hoveredHotspot) {
            ...
            const camText = `Pixel: [${cp && Array.isArray(cp) ? cp.join(", ") : "데이터 없음"
                }]`;
            const gndText = `Ground: [${gp && Array.isArray(gp) ? gp.join(", ") : "데이터 없음"
                }]`;
            crosshairTooltipElement.innerHTML = `${camText}<br>${gndText}`;
            crosshairTooltipElement.style.left = `${uploadedImageElement.offsetLeft + mouseX + 15
                }px`;
            crosshairTooltipElement.style.top = `${uploadedImageElement.offsetTop + mouseY + 15
                }px`;
    ...
    async function checkHomographyPrerequisites() {
        if (!statusGlobalAnnotationsElement || !statusCalibrationResultElement || !statusCppApiServerElement || !requestHomographyBtnElement) return;
        ...
        try {
            ...
            statusGlobalAnnotationsElement.textContent = status.hasGlobalAnnotations ? "데이터 있음 (서버 캐시)" : "데이터 없음 (서버 캐시)";
            statusGlobalAnnotationsElement.style.color = status.hasGlobalAnnotations ? "green" : "red";
    ...
    async function initializeApp() {
        ...
        if(calibrationOutputJsonElement && clientCalibrationResult){
            calibrationOutputJsonElement.value = JSON.stringify(clientCalibrationResult, null, 4);
        }
