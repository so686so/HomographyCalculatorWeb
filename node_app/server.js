...
const app = express();
const port = process.env.PORT || 3003; // Node.js 앱이 실행될 포트
...
// --- C++ API 서버 정보 설정 ---
const DEFAULT_CPP_API_HOST = "localhost"; // 로컬 개발 시 기본 호스트 (Docker 외부 실행 시)
...
const cppApiUrlFromEnv = process.env.CPP_API_URL; // docker-compose.yml에서 설정한 URL

if (cppApiUrlFromEnv) {
	try {
		const url = new URL(cppApiUrlFromEnv);
		CPP_API_HOST = url.hostname; // 예: 'cpp_api_service'
		const parsedPort = parseInt(url.port, 10); // 예: 3004
		if (
			url.protocol === "http:" &&
			CPP_API_HOST &&
			!isNaN(parsedPort) &&
			...
			console.log(
				`[Server Startup] C++ API 서버 환경 변수 CPP_API_URL 사용: Host=${CPP_API_HOST}, Port=${CPP_API_PORT}`
			);
		} else {
			// URL 형식이지만 포트 등이 유효하지 않은 경우
			throw new Error(
				`CPP_API_URL ('${cppApiUrlFromEnv}')에서 유효한 호스트 또는 포트를 추출할 수 없습니다.`
			);
		}
	} catch (error) {
		console.warn(
			`[Server Startup] 경고: CPP_API_URL ('${cppApiUrlFromEnv}') 분석 중 오류: ${error.message}. CPP_API_HOST/PORT 변수 또는 기본값을 확인합니다.`
		);
		// CPP_API_URL 파싱 실패 시, 아래 로직으로 넘어가서 CPP_API_HOST/PORT 환경변수 또는 기본값을 사용하도록 함
	}
}

if (!IS_CPP_API_CONFIGURED) {
	// CPP_API_URL이 없거나 파싱 실패 시
	const hostFromEnv = process.env.CPP_API_HOST;
	const portStrFromEnv = process.env.CPP_API_PORT;

	if (hostFromEnv && portStrFromEnv) {
		// CPP_API_HOST와 CPP_API_PORT가 둘 다 설정된 경우
		...
		if (!isNaN(parsedPort) && parsedPort > 0 && parsedPort < 65536) {
			...
			IS_CPP_API_CONFIGURED = true;
			console.log(
				`[Server Startup] C++ API 서버 환경 변수 CPP_API_HOST/PORT 사용: Host=${CPP_API_HOST}, Port=${CPP_API_PORT}`
			);
		} else {
			// 호스트는 있으나 포트가 유효하지 않은 경우 -> 기본값 사용 및 경고
			console.warn(
				`[Server Startup] 경고: CPP_API_HOST/PORT 환경 변수는 있으나 포트 ('${portStrFromEnv}')가 유효하지 않습니다. 기본값을 사용합니다.`
			);
			CPP_API_HOST = DEFAULT_CPP_API_HOST;
			CPP_API_PORT = DEFAULT_CPP_API_PORT;
			IS_CPP_API_CONFIGURED = true; // 로컬 개발 시 기본값으로라도 시도하도록 설정
		}
	} else {
		// CPP_API_URL도 없고, CPP_API_HOST/PORT 쌍도 없는 경우 -> 기본값 사용
		console.log(
			`[Server Startup] C++ API 서버 관련 환경 변수(CPP_API_URL, CPP_API_HOST/PORT) 없음. 기본값 사용: Host=${DEFAULT_CPP_API_HOST}, Port=${DEFAULT_CPP_API_PORT}`
		);
		CPP_API_HOST = DEFAULT_CPP_API_HOST;
		CPP_API_PORT = DEFAULT_CPP_API_PORT;
		IS_CPP_API_CONFIGURED = true; // 로컬 개발 시 기본값으로라도 시도하도록 설정
...
const CPP_API_HEALTH_ENDPOINT =
	process.env.CPP_API_HEALTH_ENDPOINT || "/health";
const CPP_API_HOMOGRAPHY_ENDPOINT =
	process.env.CPP_API_HOMOGRAPHY_ENDPOINT ||
	"/api/homography/calculate_dynamic"; // C++ API의 실제 Homography 연산 경로
...
function checkCppApiStatus() {
	if (!IS_CPP_API_CONFIGURED) {
		console.warn(
			"[checkCppApiStatus] C++ API 접속 정보가 유효하게 설정되지 않았을 수 있습니다. (환경 변수 확인). 상태 확인을 시도합니다."
		);
	}
	...
	return new Promise((resolve) => {
		if (!CPP_API_HOST || !CPP_API_PORT) {
			console.error(
				"[checkCppApiStatus] C++ API 호스트 또는 포트가 정의되지 않았습니다."
			);
		...
		req.on("error", (err) => {
			console.error(
				`[Server] C++ API 서버 (${CPP_API_HOST}:${CPP_API_PORT}${CPP_API_HEALTH_ENDPOINT}) 연결 오류:`,
				err.message
			);
			resolve(false);
		});
		req.on("timeout", () => {
			console.error(
				`[Server] C++ API 서버 (${CPP_API_HOST}:${CPP_API_PORT}${CPP_API_HEALTH_ENDPOINT}) 연결 시간 초과.`
			);
			req.abort();
			resolve(false);
		});
...
app.post("/api/homography/calculate", async (req, res) => {
	if (!IS_CPP_API_CONFIGURED) {
		return res
			...
			.json({
				message:
					"C++ API 서버가 올바르게 구성되지 않았습니다. 환경 변수를 확인해주세요.",
			});
	}
	...
	if (
		...
		return res
			...
			.json({
				message:
					"서버에 저장된 캘리브레이션 데이터가 유효하지 않습니다.",
			});
	}
	if (
		!globalAnnotations ||
		!Array.isArray(globalAnnotations.data) ||
		globalAnnotations.data.length < 4
	) {
		return res
			.status(400)
			.json({
				message:
					"서버에 저장된 전체 주석 데이터가 유효하지 않거나, Homography 연산을 위한 포인트 쌍(최소 4개)이 부족합니다.",
			});
	}
	...
	const payloadToCpp = {
		calibration_config: calibrationData,
		survey_data: {
	...
	console.log(
		"[Server /api/homography/calculate] C++ API 요청 페이로드:",
		JSON.stringify(payloadToCpp, null, 2)
	);
	...
	if (!cppApiOnline) {
		return res
			.status(503)
			.json({
				message: `C++ API 서버(${CPP_API_HOST}:${CPP_API_PORT})에 연결할 수 없습니다. 서버 상태를 확인하거나 나중에 다시 시도해주세요.`,
			});
	}
	...
	try {
			...
			cppRes.on("end", () => {
				try {
					...
					if (cppRes.statusCode === 200) {
						console.log(
							"[Server] Homography 연산 성공 (C++ API 응답):",
							result
						);
						res.status(200).json(result);
					} else {
						console.error(
							"[Server] Homography 연산 C++ API 오류 응답:",
							result
						);
					...
					console.error(
						"[Server] Homography 연산 C++ API 응답 파싱 오류:",
						parseError,
						"응답 내용:",
						responseBody
					);
					res.status(500).json({
						message: "C++ API 응답 처리 중 오류 발생",
		...
		cppRequest.on("error", (error) => {
			console.error(
				`[Server] C++ API (${CPP_API_HOST}:${CPP_API_PORT}${CPP_API_HOMOGRAPHY_ENDPOINT}) 요청 오류:`,
				error
			);
			res.status(500).json({
				message: "C++ API 서버 요청 중 오류 발생",
				error: error.message,
			});
		});
		...
		cppRequest.on("timeout", () => {
			console.error(
				`[Server] C++ API (${CPP_API_HOST}:${CPP_API_PORT}${CPP_API_HOMOGRAPHY_ENDPOINT}) 요청 시간 초과.`
			);
			cppRequest.abort();
		...
		res.status(500).json({
			message: "Homography 연산 처리 중 서버 내부 오류 발생",
			error: error.message,
		});
	}
});
...
app.listen(port, () => {
	...
	if (IS_CPP_API_CONFIGURED && CPP_API_HOST && CPP_API_PORT) {
		console.log(
			`C++ API 서버 연동 설정: Host=${CPP_API_HOST}, Port=${CPP_API_PORT}, Health Endpoint=${CPP_API_HEALTH_ENDPOINT}, Homography Endpoint=${CPP_API_HOMOGRAPHY_ENDPOINT}`
		);
	} else {
		console.warn(
			`[Server Startup] C++ API 연동이 올바르게 구성되지 않았습니다. Homography 연산 기능이 작동하지 않을 수 있습니다. (환경 변수 CPP_API_URL 또는 CPP_API_HOST/PORT 확인 필요)`
		);
	}
});
